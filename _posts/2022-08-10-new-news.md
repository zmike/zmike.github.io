...
With this in mind, it may seem obvious, but the above command stream is terrible for tiling GPUs. Each render pass contains only a single draw before terminating to execute a transfer operation. Sure, an argument could be made that this is running a desktop GL application on mobile, and thus any performance issues are expected, but I hate it when people make argument. Stop argumenting and get back to writing software.
...
## Even More Analysis
For another view of these drivers, let's examine the relative performance. Since GPL fast-linking is inherently a CPU task that has no relation to the GPU, it stands to reason that a CPU-based driver should be able to optimize for it the best given that there's already all manner of hackery going on to defer and delay timings. Indeed, reality confirms this, and looking at any profile of Lavapipe for the benchmark cases reveals that the only remaining bottleneck is the speed of `malloc`, which is to say the speed with which the returned pipeline object can be allocated.

...
If a driver is still slow after checking for the above items, it's time to try profiling. It's surprising what slowdowns drivers will hit. Some examples:
...
* [Lavapipe dead code deletion]()
* [Lavapipe pipeline layout reuse]()
...
Obviously it would if I'm writing a blog post about optimizing fast-linking, right?
...
NVIDIA is the gold standard for GPL fast-linking. They're pretty fast.
```
...
On another machine, here's Turnip, which advertises the fast-link feature. I've also pinned execution here to the big cores for consistency.
```
# turnip ooms with -duration
$ ./vkoverhead -start 135
...
In essence, every other driver's fast-link performance can be visualized in units of Lavapipe (`lvps`) to determine how much gain is possible if things like refactoring time and feasibility are ignored.
...
* RADV, with the insane heroics of Samuel "Gotta Go Fast" Pitoiset, has gone from roughly 0lvps last week to first place this week
* NVIDIA's fast-linking, while quite fast, is closer in performance to an unoptimized, unlanded ANV MR by 
...
You'd think by the way I just went off on my TGSI rant that I was done exploring this section, but *think again* because none of us asked what `gl_ClipDistance` or `gl_CullDistance` thought about any of this.

Well I asked, and they're not happy.

Clip/cull distance is a ~~stupid~~weird one because it's an array[8] variable that consumes two locations. And that means all the calculations/heuristics for accessing arrays that work for every other array are broken for these.

But it's fine, because this is zink and the whole thing is just a jenga tower of hacks all the way down anyway.

# Step 4: Done
I'll be honest with you, this all worked perfectly the first time I ran it.

On NVK, which, as I mentioned in my historic XDC keynote, has been testing [nir 2.0](https://gitlab.freedesktop.org/mesa/mesa/-/merge_requests/24432) since last year. Truly a driver living in the future.

Other drivers, however, required considerable more work to make CI not explode. The clear choice once parity was achieved was to then start deleting code.
...
I didn't even get to mention the great things that happened related to maintenance5 yet. Be sure to read again next week when I inevitably shovel more garbage onto the internet in the form of an unfortunately large blog post.
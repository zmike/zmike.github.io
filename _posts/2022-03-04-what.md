...
## Debugging
How does one debug JIT code? GDB can't be used, valgrind doesn't work, and, despite what LLVM developers would tell you, building an assert-enabled LLVM doesn't help at all in most cases here since that will only catch invalid behavior, not valid behavior that very obviously produces invalid results.

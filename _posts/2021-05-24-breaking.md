...
But I also encountered a bug so bizarre, so infruating, so esoteric, that I need to take a bit of a victory lap now that I've successfully corralled it. So let's get into a real, vintage SGC blog post like we used to have back when SGC was a good blog.

...
Thus for an ES context, the point size always comes from the last vertex stage, which means it can be anything it wants to be if that stage is a vertex shader and **cannot be written to for all other stages because it is not a valid output** (this bolded part is going to be really funny in a minute or two).
...
CTS explicitly has "wide points" tests which verify illegal point sizes that are exported by the tessellation and geometry shader stages. Also, let's be reasonable people for a moment, who actually wants a point that's just one pixel? Nobody can see that on their 8k display.